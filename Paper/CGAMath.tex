...
So what are the transformations?  Since versors are geometric products of vectors,
we first look at what vectors do.  All other transformations are composed of a sequence
of what transformations vectors can perform.  What we find is that vectors can
perform reflections of inversions.  Interestingly, all rigid body transformations come
from just reflections.

\subsection*{Reflections}

If we let $V$ be the dot representation of a plane, then $Vp(c)\tilde{V}$ reflects $c$ about the plane.
If $x$ is the smallest vector such that $c+x$ is on the plane, then the reflected point is $c + 2x$.

\subsection*{Inversions}

If we let $V$ be the dot representation of a sphere, then $Vp(c)\tilde{V}$ inverts $c$ about the sphere.
But what in the world does this mean?  Is the point reflected across the surface of the sphere?  That was my
initial thought, but to really know what's going on, we need to do the math.

If we let $V=\nvao-\frac{1}{2}r^2\nvai$ be the dot representation of a sphere centered at origin with radius $r$, then
\begin{equation*}
\frac{Vp(c)\tilde{V}}{-c^2} = p(r^2c^{-1}).
\end{equation*}
Here we see where the transform gets its name.  Seeing where the point is going to land in all cases is still
not terribly intuitive with this result.

Note that it's easy to see what happens if we choose a sphere with center not at the origin.
Let $T$ be a translation versor that gets us from the origin to the chosen sphere center.
Then what we're considering is
\begin{equation*}
(TVT^{-1})p(c)(TVT^{-1})^{-1} = T(V(T^{-1}p(c)T)V)T^{-1}.
\end{equation*}
So we see that the problem is just translated to origin where our original analysis holds, then we
translate back out.  In short, spherical inversions at origin act the same way anywhere else, and
that place is determined by the sphere's center.

\subsection*{Rotations}

These are just double reflections in distinct planes intersecting in the axis of rotation.
The angle between the planes is half the desired angle of rotation.

\subsection*{Translations}

These are just double reflections in distinct yet parallel planes.
The distance between the planes is half the desired distance of translation.

\subsection*{Uniform Scale}

These are also called diolations.
What we do here is a double inversion about concentric spheres.
If
\begin{equation*}
V=(\nvao+\frac{1}{2}(-r_b^2)\nvai)(\nvao+\frac{1}{2}(-r_a^2)\nvai),
\end{equation*}
...
Note that the argument we made for inversions translating anywhere applies here as well.
In other words, we can diolate, not just from origin, but from anywhere else, and the place is
determined by the center of the two concentric spheres.

At this point it's very natural to ask: if we found value in double reflections where the two
planes involved changed in their relative special relationship (i.e., parallel versus intersecting),
might we do that here as well?
That is, how about a double inversion between two non-concentric spheres?
